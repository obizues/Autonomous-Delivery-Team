from ai_software_factory.agents.agent_config import AGENT_CONFIG
...
class WorkflowEngine:
    def __init__(
        ...
        self.agents: Mapping[str, Agent] = agents
        self.approval_service: ApprovalService = approval_service
        self.escalation_service: EscalationService = escalation_service
        self.max_revisions: int = max_revisions
        from ai_software_factory.governance.policy import PolicyManager
        self.policy_manager = policy_manager or PolicyManager()
        self.repo_paths: list[str] = repo_paths or []
    ...
    def start(self, backlog_item) -> WorkflowState:
        """
        ...
        priority: Any | str = getattr(backlog_item, "business_value", "")
        if hasattr(backlog_item, "acceptance_criteria"):
            priority_score: int = len(backlog_item.acceptance_criteria)
        else:
            priority_score = 1
        # Assign to agent (simple: product_owner or engineer)
        assigned_agent: str = "engineer" if priority_score > 2 else "product_owner"
        workflow_id = str(uuid.uuid4())
        # Persist the BacklogItem as an artifact so it is always available to the UI
        backlog_item.workflow_id = workflow_id
    ...
    def latest_escalation(self, workflow_id: str) -> EscalationArtifact | None:
        artifacts: list[BaseArtifact] = self.artifact_store.list_by_workflow(workflow_id)
        escalations: list[EscalationArtifact] = [artifact for artifact in artifacts if isinstance(artifact, EscalationArtifact)]
    ...
    def _latest_gate_feedback_decisions(self, workflow_id: str) -> dict[WorkflowStage, Decision]:
        ...
        for artifact in artifacts:
            ...
            if not is_review_gate(artifact.stage):
                continue
            existing: tuple[int, Decision] | None = latest.get(artifact.stage)
    ...
    def resume_from_escalation(
        ...
        escalation = None
        # Try to find escalation artifact matching the current or resume stage
        for artifact in self.artifact_store.list_by_workflow(workflow_id):
            if isinstance(artifact, EscalationArtifact) and getattr(artifact, "stage", None) == (resume_stage or state.current_stage) and getattr(artifact, "escalation_status", None) != "RESOLVED":
                escalation = artifact
                break
        # If not found, fall back to the most recent unresolved escalation artifact for this workflow
        if escalation is None:
            unresolved: list[EscalationArtifact] = [a for a in self.artifact_store.list_by_workflow(workflow_id) if isinstance(a, EscalationArtifact) and getattr(a, "escalation_status", None) != "RESOLVED"]
            if unresolved:
                # Use the most recent (by created_at or artifact_id as fallback)
                escalation = sorted(unresolved, key=lambda a: getattr(a, "created_at", None) or getattr(a, "artifact_id", ""))[-1]
        if escalation is None:
    ...
    def execute_next(self, workflow_id: str) -> WorkflowState | None:
        ...
        if state.status == WorkflowStatus.ESCALATED:
            ...
            escalation_artifacts: list[BaseArtifact] = [a for a in self.artifact_store.list_by_workflow(state.workflow_id) if getattr(a, "stage", None) == state.current_stage and getattr(a, "escalation_status", None) == "RESOLVED"]
        ...
        if min_artifacts is not None:
            artifacts: list[BaseArtifact] = self.artifact_store.list_by_workflow(state.workflow_id)
        ...
        roles: list[str] = STAGE_TO_ROLE.get(stage, [])
        if not roles:
            latest_gate_decisions: dict[WorkflowStage, Decision] = self._latest_gate_feedback_decisions(state.workflow_id)
        ...
        # Multi-agent collaboration: all agents for this stage act and their outputs are merged
        all_results = []
        for role in roles:
            agent = self.agents[role]
            agent_config = AGENT_CONFIG.get(role, {})
            ...
            all_results.append((role, result))

        # Merge produced artifacts, decisions, and escalations from all agents
        for role, result in all_results:
            for artifact in result.produced_artifacts:
                ...
                if isinstance(artifact, PullRequest):
                    ...
                    # Push code to remote UI repo after PR artifact is created
                    try:
                        # Assumes repo_workspace_manager is available and configured
                        if hasattr(self, 'repo_workspace_manager') and getattr(self, 'repo_workspace_manager', None) is not None:
                            workflow_id = state.workflow_id
                            commit_message = f"Automated commit for PR: {artifact.title}"
                            getattr(self, 'repo_workspace_manager').push_to_remote(workflow_id, commit_message=commit_message)
                    except Exception as e:
                        # Only emit if ERROR is a valid EventType, else fallback to string
                        event_type = getattr(EventType, 'ERROR', 'ERROR')
                        self.event_bus.emit(
                            workflow_id=state.workflow_id,
                            event_type=event_type,
                            stage=stage,
                            payload={"error": str(e), "context": "push_to_remote"},
                        )
            ...
            if result.decision is not None:
                self.event_bus.emit(
                    ...
                    stage=stage,
                    payload={"decision": result.decision.value, "notes": result.notes, "role": role},
                )

            if is_review_gate(stage) and result.decision is not None:
                approval: ApprovalRecord = self.approval_service.create_approval(
                    workflow_id=state.workflow_id,
                    stage=stage,
                    reviewer=role,
                ...
                self.event_bus.emit(
                    ...
                    stage=stage,
                    payload={"approval_id": approval.approval_id, "decision": approval.decision.value, "role": role},
                )
                ...
                if result.decision == Decision.REQUEST_CHANGES:
                    ...
                    regression_detected: bool = not bool(progress.get("no_new_failures", True))

                    if state.revision >= self.max_revisions:
                        escalation: EscalationArtifact = self._record_escalation(
                            state=state,
                            stage=stage,
                            reason=(
        ...
        # Optionally, emit all agent results for transparency
        for role, result in all_results:
            self.event_bus.emit(
                workflow_id=state.workflow_id,
                event_type=EventType.STAGE_COMPLETED,
                stage=stage,
                payload={"stage_result": asdict(result), "role": role},
            )
        return state
    ...
    def _build_progress_summary(self, workflow_id: str) -> dict:
        ...
        if hasattr(self.artifact_store, 'list_by_workflow'):
            tests: list[BaseArtifact] = [artifact for artifact in self.artifact_store.list_by_workflow(workflow_id) if hasattr(artifact, 'failed_cases')]
        if not tests:
            return {
                "failures_reduced": 0,
        ...
        latest: BaseArtifact = tests[-1]
        previous: BaseArtifact | None = tests[-2] if len(tests) >= 2 else None
        failures_reduced = int(getattr(latest, "failures_reduced", 0))
        if previous is not None and failures_reduced == 0:
            failures_reduced: int = max(0, int(getattr(previous, "failed_cases", 0)) - int(getattr(latest, "failed_cases", 0)))
        no_new_failures: bool = not bool(getattr(latest, "regression_detected", False))
        stable_pass_streak = int(getattr(latest, "stable_pass_streak", 1 if getattr(latest, "passed", False) else 0))
        stalled = False
        if len(tests) >= 2:
            newest: BaseArtifact = tests[-1]
            older: BaseArtifact = tests[-2]
            if (
